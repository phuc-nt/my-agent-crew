...
def today_conversation(deps: AgentDeps, chat_id: int, clock: Any) -> Conversation:
    ...
    if latest is None or datetime.fromisoformat(latest.created_at).astimezone().date() != today:
        return open_conversation(deps, chat_id, clock)
    return latest


def open_conversation(deps: AgentDeps, chat_id: int, clock: Any) -> Conversation:
