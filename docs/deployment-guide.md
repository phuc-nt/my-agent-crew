...
**Tuyến mô hình chung** (tuyến agent không có tuyến riêng dùng, và tuyến agent có tuyến riêng quay về khi không tuyến nào của nó có khoá) sửa ở thẻ Tuyến mô hình cùng trang: thêm/bớt/đổi thứ tự rồi **Lưu tuyến**. Server từ chối (409) nhà cung cấp chưa có khoá và danh sách đội không chạy được, rồi mới ghi `routes` vào `<home>/config.yaml` kiểu round-trip (giữ chú thích và khoá khác) và áp dụng từ lượt kế tiếp. `/api/connections` trả `routes_source`: `env` khi `MY_AGENT_ROUTES` đang đặt — biến thắng tệp nên trang chỉ cho xem —, `config` hoặc `default`. Xoá khoá mà tuyến duy nhất cần bị từ chối kèm tên tuyến dạng `provider:model` và cách gỡ (thêm tuyến khác trước).
