...
| Delegated work in the UI: a `delegate` call reads as the task it is (who took it, cost, steps) with the child's reply instead of the result header, a running one offers nothing to open yet, a work agent's conversation is badged with its cap and step budget, spend says how many children it covers, a conversation an agent opened is marked and sorted below the person's own, a delegated run nests under the run that asked for it, and settings lists the crew's roles and the command that installs a bundled profile | — | `delegate-cards.test.tsx`, `delegate-result.test.ts`, `activity-reducer.test.ts` | `delegate-smoke.spec.ts` |
